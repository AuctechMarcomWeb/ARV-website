(function ($) {
	...
	/* ── Gallery Page — Dynamic Loader with Pagination ── */
	if ($("#galleryRow").length) {
		var GALLERY_PER_PAGE = 12;
		var galleryAllImages = [];   // flat list of all {url, alt} objects
		var galleryCurrentPage = 1;

		function galleryRenderPage(page) {
			var $row  = $("#galleryRow");
			var $pagi = $("#galleryPagination");
			var $list = $("#galleryPaginationList");

			var totalImages = galleryAllImages.length;
			var totalPages  = Math.ceil(totalImages / GALLERY_PER_PAGE);
			var start       = (page - 1) * GALLERY_PER_PAGE;
			var end         = Math.min(start + GALLERY_PER_PAGE, totalImages);
			var pageImages  = galleryAllImages.slice(start, end);
			...
			pageImages.forEach(function (img, i) {
				...
				$row.append([
					...
					'    <a href="' + img.url + '" data-cursor-text="View">',
					'      <figure>',
			...
			if (totalPages <= 1) {
				...
				$list.append(
					...
					'  <a href="#" data-page="' + (page - 1) + '" aria-label="Previous">' +
				...
				$list.append(
					...
					'  <a href="#" data-page="' + (page + 1) + '" aria-label="Next">' +
			...
			// ── Reinit Magnific Popup for fresh cards ──
			$('.gallery-items').magnificPopup('destroy');
			$('.gallery-items').magnificPopup({
				delegate: 'a',
				type: 'image',
				closeOnContentClick: false,
			...
			// ── Reinit WOW for new cards ──
			new WOW().init();
		}

		// ── Pagination click handler (delegated) ──
		$(document).on("click", "#galleryPaginationList a", function (e) {
			e.preventDefault();
			var $li    = $(this).parent();
			if ($li.hasClass("disabled") || $li.hasClass("active")) return;

			var targetPage = parseInt($(this).data("page"), 10);
			var totalPages = Math.ceil(galleryAllImages.length / GALLERY_PER_PAGE);
			if (isNaN(targetPage) || targetPage < 1 || targetPage > totalPages) return;

			galleryCurrentPage = targetPage;
			galleryRenderPage(galleryCurrentPage);

			// Scroll to top of gallery section
			$("html, body").animate({ scrollTop: $("#galleryRow").offset().top - 100 }, 400);
		});
			...
			.then(function (response) {
				...
				items.forEach(function (item) {
					...
					urls.forEach(function (imgUrl) {
						...
						galleryAllImages.push({ url: imgUrl, alt: altText });
					});
				});

				if (!galleryAllImages.length) return;
				galleryRenderPage(galleryCurrentPage);
			})
			.catch(function (err) {
